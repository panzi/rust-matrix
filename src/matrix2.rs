...
use crate::assert::{Assert, IsTrue, TypeEq};
...
//    #[inline]
//    pub fn into_transpose(mut self) -> Matrix<Y, X, T, D>
//    where [T; Y * X]: Sized, TypeEq<D, Box<[T; X * Y]>>: IsTrue {
//        for y in 1..Y {
//            let yoffset = y * X;
//            for x in 0..X {
...
impl<const X: usize, const Y: usize, T: Number, D: AsRef<[T; X * Y]>> AsRef<Self> for Matrix<X, Y, T, D>
where [T; X * Y]: Sized
{
    #[inline]
    fn as_ref(&self) -> &Self {
...
impl<const X: usize, const Y: usize, T: Number, D: AsRef<[T; X * Y]>> AsMut<Self> for Matrix<X, Y, T, D>
where [T; X * Y]: Sized
{
    #[inline]
...
impl<const X: usize, const Y: usize, T: Number, D: AsRef<[T; X * Y]>> AsRef<[[T; X]]> for Matrix<X, Y, T, D>
where [T; X * Y]: Sized
{
    #[inline]
    fn as_ref(&self) -> &[[T; X]] {
        unsafe { self.data.as_ref().as_chunks_unchecked::<X>() }
    }
}

impl<const N: usize, T: Number, D: AsRef<[T; N * N]>> Unit for Matrix<N, N, T, D>
where [T; N * N]: Sized {
    #[inline]
    fn unit() -> Self {
...
impl<const X: usize, const Y: usize, T: Number, D: AsRef<[T; X * Y]>> Default for Matrix<X, Y, T, D>
where [T; X * Y]: Sized
{
    #[inline]
    fn default() -> Self {
...
impl<const X: usize, const Y: usize, T: Number, D: AsRef<[T; X * Y]>> PartialEq<&[[T; X]; Y]> for Matrix<X, Y, T, D>
where T: PartialEq, [T; X * Y]: Sized {
    #[inline]
    fn eq(&self, other: &&[[T; X]; Y]) -> bool {
        let other: &[T; X * Y] = unsafe { std::mem::transmute(*other) };
...
impl<const X: usize, const Y: usize, T: Number, D: AsRef<[T; X * Y]>> PartialEq<Matrix<X, Y, T, D>> for &[[T; X]; Y]
where T: PartialEq, [T; X * Y]: Sized {
    #[inline]
    fn eq(&self, other: &Matrix<X, Y, T, D>) -> bool {
